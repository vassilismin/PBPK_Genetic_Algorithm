...
set.seed(8788)
fitted_GAFP <-  create.position(position_GAFP)$fitted
...
params <- c(phys_pars, position_GAFP, params_GAFP)
solution <- data.frame(ode(times = sample_time,  func = ode.func,
                           y = inits, parms = params, 
                           method="bdf",rtol = 1e-5, atol = 1e-5))

solution_GAFP <- data.frame(solution$time,solution$Blood, solution$C_ht, solution$C_lu,
                             solution$C_li, solution$C_spl, solution$C_ki,
