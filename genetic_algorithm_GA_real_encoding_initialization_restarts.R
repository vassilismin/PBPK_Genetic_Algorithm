# Here the GA select based on the new fitness metric
...
{ 
  ...
  nm_values <- rep(NA,5)
  for (i in 1:5){
  fit[] <- c(log(exp(runif(P_groups, 3,6))),log(exp(runif(X_groups+3, -3,1))))
  try(
...
GA_results <- GA::ga(type = "real", fitness = ga_fitness, 
                     ...
                     monitor =plot,
                     seed = 8080)
save.image(file = "ga_real_initialization_restarts.RData")