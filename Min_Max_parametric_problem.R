...
# Minimum problem (x_gen, P_gen, CLE_f, CLE_u)
...
start_time <- Sys.time()
nm_optimizer_min <- nmk(par = x0, fn = obj.func,
                    control = list(maxfeval=1000, trace=TRUE))
end_time <- Sys.time()
opt_time_min <- end_time - start_time 



#Maximum problem: 1 x and 1 P for every compartment, CLE_f, CLE_u
problem <- "max"
set.seed(0)
x0 <- log(runif(18, 1e-05,10))
names(x0) <- c("x_ht", "x_lu", "x_li", "x_spl",
               "x_ki", "x_git", "x_bone", "x_rob", 
               "P_ht", "P_lu", "P_li", "P_spl",
...
end_time <- Sys.time()
opt_time_max <- end_time - start_time 

# BFGS optimizer for the max problem
problem <- "max"
set.seed(0)
x0 <- log(runif(18, 1e-05,10))
names(x0) <- c("x_ht", "x_lu", "x_li", "x_spl",
               ...
               "CLE_f", "CLE_u")
start_time <- Sys.time()
bfgs_optimizer_max <- optim(par = x0, fn = obj.func, method = c("BFGS"),
                               control = list(trace=1, maxit=1e+4))
end_time <- Sys.time()
bfgs_time_max <- end_time - start_time
